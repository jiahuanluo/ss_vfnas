...
def train(train_queue, model_A, model_B, criterion, optimizer_A, optimizer_B, epoch):
    ...
    for step, (trn_X_A, trn_X_B, trn_y) in enumerate(train_queue):
        ...
        logits = model_A(input_A, U_B_noised)
        loss = criterion(logits, target)
        U_B_gradients = torch.autograd.grad(loss, U_B_noised, retain_graph=True)
        U_B_gradients_noised = add_dp_v3(U_B_gradients[0], args.clip_norm_grad, args.noise_variance_grad)
        model_B_weights_gradients = torch.autograd.grad(U_B, model_B.parameters(), grad_outputs=U_B_gradients_noised,
                                                        retain_graph=True)
